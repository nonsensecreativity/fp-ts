...
function getLoggers(logs: Array<string | number>) {
  const log = (s: string | number) => new IO(() => { logs.push(s) })
...
describe('AIO', () => {
  ...
  it('chain', () => {
    ...
    let counter = 0
    const aio2 = liftIO(new IO(() => {
      counter += 1
      return 'a'
    }))
    const aio3 = liftIO(new IO(() => {
      counter += 1
      return 'b'
    }))
  ...
  it('fromTask', () => {
    let counter = 0
    const aio = fromTask(new Task(() => new Promise<string>(resolve => {
      counter += 1
      resolve('a')
    })))
    return toPromise(aio).then(a => {
