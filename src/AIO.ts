// Adapted from https://github.com/slamdata/purescript-aff (PureScript Aff monad)
...
export function attempt<A>(fa: AIO<A>): AIO<Either<Error, A>> {
  return new AIO<Either<Error, A>>((_, s) => fa.run(
    err => s(left<Error, A>(err)),
    a => s(right<Error, A>(a))
  ))
}
...
/** Lifts a task computation */
export function fromTask<A>(task: Task<A>): AIO<A> {
  return new AIO<A>((e, s) => {
    task.run()
      .then(a => s(a).run())
      .catch(err => e(err).run())
...
export function toTask<A>(aio: AIO<A>): Task<A> {
  return new Task<A>(() => toPromise(aio))
}
